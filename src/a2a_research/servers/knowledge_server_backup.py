    ...
    def _setup_routes(self):
        ...
        async def analyze_credibility(
            ...
            if self.use_llm:
                ...
                print(f\"[knowledge-server] ❌ LLM not available for credibility analysis\")
                analysis = {\"error\": \"LLM not available\"}
...
Extract 15-25 insights total. Be thorough and comprehensive."""
            ...
            print(f"[knowledge-server] ✅ Mistral extracted {len(insights)} insights from: {url[:50]}...")
            return insights
            
        except Exception as e:
            print(f"[knowledge-server] ❌ Mistral extraction failed: {e}")
            return self._extract_insights_with_regex(result_data)
